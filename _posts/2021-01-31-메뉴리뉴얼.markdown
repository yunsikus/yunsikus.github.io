...
손님 번호	주문한 단품메뉴 조합
![menu_comb](https://yunsikus.github.io/assets/img/post_img/메뉴리스트1.jpeg)
...
코스 종류	메뉴 구성	설명
![menu_comb2](https://yunsikus.github.io/assets/img/post_img/메뉴리스트2.jpeg)
...
def do(order, start, k):
    ...
    # 백트래킹을 이용하여 조합 추출 ['A','B','C] -> AB, AC, BC
    # BA, CA, CB는 중복값이라 제외시키기 위해 range를 (start, len(order))로 해줌
    # check 리스트로 중복 제거
...
def solution(orders, course):
    ...
    for cs_len in course: # [2,3,4]
        ...
        answer.extend([k for k,v in count_dict.items() if v == count_dict.most_common(1)[0][1] and v >= 2]) # 최대 빈도 value를 가지는 key 추출
